...
Import the entire random module to use its functions and classes with the random prefix.
"""
...
Create a Tkinter application window and define its dimensions and title.

Set the height of the window to 500 pixels and the width to 800 pixels.
...
Description:
You are tasked with implementing rocket movement in a graphical application using Tkinter. The rocket can be controlled using the arrow keys to navigate it up, down, left, or right within the canvas.
...
def create_asteroid():
    ...
    asteroid = random.choice(asteroid_img)
